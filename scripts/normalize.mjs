/**
 * Figma 플러그인 JSON → DTCG 형식 변환 스크립트
 *
 * 실행: node scripts/normalize.mjs
 * 입력: tokens/color.tokens.json (Figma 플러그인 출력)
 * 출력: tokens/color.normalized.tokens.json (플랫폼별 빌드 입력용)
 *
 * 변환 규칙:
 * - Figma 변수는 자연스러운 이름으로 모두 추가
 * - 기존 웹 변수 중 Figma와 번호가 다른 것은 이름 유지, 값만 대응 Figma 토큰 값으로 변경
 *   ($color-rgba-dark-54 → BlackAlpha/50 값, -68 → BlackAlpha/70 값, $color-rgba-light-84 → WhiteAlpha/80 값)
 */
...
const figmaJson = JSON.parse(readFileSync(resolve(TOKENS_DIR, 'color.tokens.json'), 'utf-8'));
...
for (const [group, tokens] of Object.entries(figmaJson)) {
